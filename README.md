...
`doc/` 仅作实现参考，不参与构建。
无设计稿的页面按批次自绘，规矩见 [§二.5](#5-无设计稿页面12-页)。
...
- **无设计稿自绘 12 页** —— 依设计系统在既有布局范式内实现，见 §5
...
其余 **10 个二级**（税源监控 3 / 智能应用 3 / 系统管理 4）统一落 `PlaceholderView`。
...
| 3 | 风险管理 | 任务派发 · 结果回填 · 处置绩效统计 | 列表 + 看板;回填页**整体复用** `DisposalForm`,不另写一套表单 |
| 4 | 规则库管理 | 规则试跑与灰度 · 阈值参数管理 · 规则效果监测 | 与「规则配置」刻意区分视角:配置页是**单规则**（定义/阈值/效果三标签）,本组是**跨规则**（试跑队列 / 全库参数审计 / 全库效果横向对比） |
| 5 | 智能模型 | 风险评分模型 · 虚开团伙识别 · 异常申报检测 · 行业税负基准 | 见下 |
...
接口按业务分组（11 组）：`dashboard` / `clues` / `archive` / `rules` / `qa` / `graph` /
`decision` / `datagov` / `riskmgmt` / `ruleops` / `model`，各组方法签名由 `ApiClient` 统一约束。
...
| — | 税源监控 3 · 智能应用 3 · 系统管理 4 | ⬜ 占位页 |

合计 **24 个业务页**（其中 10 页 1:1 复刻自设计稿），菜单 `done 25 / placeholder 10`
（25 含首页驾驶舱在内的全部叶子节点）。
...
`components/clues/`（`DisposalForm` —— 核查处置与结果回填共用）、
