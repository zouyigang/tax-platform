...
| 2 | **风险线索工作台** | `/#/clues` | 风险线索列表(高密度)、详情抽屉、派发/退回、处置结果回填 | ✅ 已实现 |
| 3 | 风险规则配置 | `/#/rules` | 规则库增删改、判定条件编辑、调度频率、启用开关、命中效果监测 | ⬜ 占位页 |
| 4 | 核查任务管理 | `/#/tasks` | 核查任务分派与进度跟踪 | ⬜ 设计稿待建 |
| 5 | **一户式档案** | `/#/archive` | 纳税人全景档案，六大类：基础/登记/经营/申报/票流/征管评价 | ✅ 已实现 |
...
| 层级 | 位置 | 承载内容 |
|---|---|---|
| **全局设计系统** | `src/styles/tokens.css` | 《设计系统》定义的通用值：主色四档、辅色、深色表面 6 色、9 级中性灰阶、风险三级与状态三态（含文字色与底纹色）、8 色图表序列、字号字重行高、4px 基数间距刻度、圆角/描边/阴影、动效时长 |
...
| 页面 | 状态 |
|---|---|
| 领导驾驶舱 / 风险线索工作台 / 一户式档案 | ✅ 已实现 |
| 风险规则配置 / 关联图谱分析 / 政策智能问答 | ⬜ 占位页 |
